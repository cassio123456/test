bitcoinEvelin Core version *0.15.1* is now available from:

  <https://download.bitcoinevelin.org/bitcoinevelin-0.15.1/>
...
A number of changes to the way bitcoinEvelin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 BTCev). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 BTCev) in the next release.
