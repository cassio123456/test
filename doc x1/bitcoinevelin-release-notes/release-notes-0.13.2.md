bitcoinEvelin Core version 0.13.2 is now available from:

  <https://download.bitcoinevelin.org/bitcoinevelin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/bitcoinevelin-project/bitcoinevelin/issues>
...
updates will be released anymore. Without security updates, using a bitcoinevelin
...
ECDSA signatures inside bitcoinEvelin transactions now use validation using
...
Previous versions of bitcoinEvelin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of bitcoinEvelin Core was to raise the default minimum
relay fee.

bitcoinEvelin Core 0.13.2 will have a strict maximum size on the mempool. The
...
bitcoinEvelin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running bitcoinevelind without having to do any manual configuration.
...
bitcoinEvelin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. bitcoinEvelin Core will relay transactions with
...
entirely in bitcoinEvelin Core 0.13, and it is at this time undecided whether the
...
bitcoinEvelin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
bitcoinEvelin Core automatically creates a hidden service to listen on, without
manual configuration. bitcoinEvelin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if bitcoinEvelin Core is listening, and
...
bitcoinEvelind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals bitcoinEvelin
Core to use floating fees. By default, bitcoinEvelin Core will use floating
fees.
...
(default: `0.0002` BTCev/kB).

At all times, bitcoinEvelin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTCev.
Furthermore, bitcoinEvelin Core will never create transactions paying less than
...
`bitcoinevelin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinevelinrpc>
        ...
        # optional bypass bitcoinevelind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< bitcoinevelinrpc:<password>
    </Location>
...
Restarting bitcoinevelind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in bitcoinevelin.conf
...
bitcoinevelin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bitcoinevelin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The bitcoinEvelin Core code base has
...
- `bitcoinevelin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `bitcoinevelin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the bitcoinEvelin scripting system that in combination with
...
bitcoinEvelin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of bitcoinEvelin Core.
...
The two phases are distinct in the bitcoinEvelin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming bitcoinEvelin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for bitcoinEvelin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each bitcoinEvelin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bitcoinevelins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bitcoinevelins being spent is exactly
...
- **Increased security for multisig:** bitcoinEvelin addresses (both P2PKH addresses
  ...
  believed to be provided by bitcoinEvelin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of bitcoinEvelin's current ledger (called the UTXO set),
  ...
  note that bitcoinEvelin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  bitcoinEvelin users to individually opt-in to almost any change in the bitcoinEvelin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and bitcoinEvelin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the bitcoinEvelin
client to bitcoinEvelin Core 0.13.2 to signal for segwit support. When a
...
Since bitcoinEvelin Core 0.10.0, nodes have defaulted to only relaying and
...
- `bitcoinevelin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bitcoinevelin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
